...
type Dimensions={length:number;width:number;height:number;weight:number;dimensionUnit:string;weightUnit:string;valid?:boolean};
type Product={id:string;offerId?:string|null;title:string;description:string;sku?:string|number|null;dimensions?:Dimensions};
...
export default function MarketplacesPage(){
  ...
  const saveProduct=async(p:Product)=>{if(!confirm(`Обновить карточку «${p.title}» и габариты упаковки?`))return;setBusy('product:'+p.id);setMsg('');const res=await fetch('/api/admin/marketplaces/product',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({marketplace:mp,id:p.id,offerId:p.offerId,title:mp==='wb'?p.title:undefined,description:p.description,dimensions:p.dimensions})});const j=await res.json();if(res.ok){setMsg('✓ Карточка и габариты отправлены на маркетплейс');await loadProducts()}else setMsg(j.error||'Ошибка обновления');setBusy('')};
  const setDim=(idx:number,key:keyof Dimensions,value:any)=>setProducts(a=>a.map((x,i)=>i===idx?{...x,dimensions:{...(x.dimensions||{length:0,width:0,height:0,weight:0,dimensionUnit:mp==='wb'?'cm':'mm',weightUnit:mp==='wb'?'kg':'g'}),[key]:value}}:x));
  ...
  const fieldStyle={border:'1px solid #ded4ca',borderRadius:10,padding:'10px 11px',width:'100%'} as any;
  ...
  return <main className="admin-shell">
    ...
    {configured&&tab==='products'&&<section>
      ...
      {products.map((p,i)=><article className="admin-card editor" key={`${p.id}-${i}`}>
        <div className="editor-head"><div><h2 style={{fontSize:18}}>{p.title||p.offerId}</h2><span>ID: {p.id}{p.offerId?` · Offer: ${p.offerId}`:''}</span></div></div>
        <div className="form-grid">{mp==='wb'&&<label className="wide">Название<input value={p.title} maxLength={200} onChange={e=>setProducts(a=>a.map((x,idx)=>idx===i?{...x,title:e.target.value}:x))}/></label>}<label className="wide">Описание<textarea style={{minHeight:150}} value={p.description||''} maxLength={10000} onChange={e=>setProducts(a=>a.map((x,idx)=>idx===i?{...x,description:e.target.value}:x))}/></label></div>
        <div style={{marginTop:16,padding:16,border:'1px solid #e6ddd4',borderRadius:12}}><div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',marginBottom:12}}><div><b>Габариты упаковки</b><div><small>{mp==='wb'?'WB: сантиметры и килограммы':'Ozon: миллиметры и граммы'}</small></div></div>{p.dimensions?.valid===false&&<b style={{color:'#a15b18'}}>WB считает габариты некорректными</b>}</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:10}}><label>Длина ({p.dimensions?.dimensionUnit|| (mp==='wb'?'cm':'mm')})<input type="number" min="0.01" step={mp==='wb'?'0.1':'1'} style={fieldStyle} value={p.dimensions?.length??0} onChange={e=>setDim(i,'length',Number(e.target.value))}/></label><label>Ширина ({p.dimensions?.dimensionUnit|| (mp==='wb'?'cm':'mm')})<input type="number" min="0.01" step={mp==='wb'?'0.1':'1'} style={fieldStyle} value={p.dimensions?.width??0} onChange={e=>setDim(i,'width',Number(e.target.value))}/></label><label>Высота ({p.dimensions?.dimensionUnit|| (mp==='wb'?'cm':'mm')})<input type="number" min="0.01" step={mp==='wb'?'0.1':'1'} style={fieldStyle} value={p.dimensions?.height??0} onChange={e=>setDim(i,'height',Number(e.target.value))}/></label><label>Вес ({p.dimensions?.weightUnit|| (mp==='wb'?'kg':'g')})<input type="number" min="0.001" step={mp==='wb'?'0.001':'1'} style={fieldStyle} value={p.dimensions?.weight??0} onChange={e=>setDim(i,'weight',Number(e.target.value))}/></label></div></div>
        <div className="editor-actions"><button className="save-btn" disabled={busy!==''} onClick={()=>saveProduct(p)}>{busy==='product:'+p.id?'Сохраняю…':'Сохранить карточку и габариты'}</button></div>
      </article>)}
    </section>}

    {tab==='settings'&&<section className="admin-card editor"><h2>Подключение API</h2><p>WB: <b>{status?.wb?.configured?'✓ подключён':'не подключён'}</b> · Ozon: <b>{status?.ozon?.configured?'✓ подключён':'не подключён'}</b> · AI: <b>{status?.ai?.configured?'✓ OpenAI':'шаблонные ответы'}</b></p><div style={{display:'grid',gap:8,background:'#2c241d',color:'#fff',padding:16,borderRadius:12}}><code>WB_API_TOKEN=...</code><code>WB_FEEDBACK_TOKEN=...</code><code>OZON_CLIENT_ID=...</code><code>OZON_API_KEY=...</code><code>OPENAI_API_KEY=...</code><code>OPENAI_MODEL=gpt-5.6-luna</code><code>OZON_DESCRIPTION_ATTRIBUTE_ID=...</code></div><p>Карточки WB используют WB_API_TOKEN с правами «Контент». WB_FEEDBACK_TOKEN используется только для отзывов/вопросов.</p><button className="edit-btn" onClick={loadStatus}>Проверить настройки</button></section>}
  </main>;
}
