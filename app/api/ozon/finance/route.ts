...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const by:Record<string,{count:number;amount:number}>={};const sku:Record<string,{sku:string;title:string;qty:number;sales:number;fees:number;net:number}>={};let total=0,sales=0,services=0,points=0; const add=(name:string,value:any)=>{const amount=num(value);if(!amount)return;details[name]??={count:0,amount:0};details[name].count++;details[name].amount+=amount;};
  for(const a of all){const amount=num(a.total_amount);total+=amount;const id=String(a.accrual_id??a.type_id??"other");const name=types[id]??a.accrued_category??("Начисление "+id);by[name]??={count:0,amount:0};by[name].count++;by[name].amount+=amount;
   const p=a?.posting?.products??[];for(const x of p){
    const sale=num(x?.sale?.seller_price??x?.seller_price); sales+=sale;
    let fees=num(x?.delivery?.total_accrued);
    for(const fg of(x?.item_fees?.fees??[]))for(const fee of(fg?.fees??[]))fees+=num(fee?.accrued);
    services+=fees;
    const code=String(x?.offer_id??x?.sku??x?.product_id??"Без SKU");
    sku[code]??={sku:code,title:String(x?.name??x?.title??code),qty:0,sales:0,fees:0,net:0};
    sku[code].qty+=Number(x?.quantity??1)||1;sku[code].sales+=sale;sku[code].fees+=fees;sku[code].net+=sale+fees;
   }
   const non=num(a?.non_item_fee?.accrued);services+=non;for(const cf of(a?.container_fees??[]))services+=num(cf?.accrued??cf?.total_amount);
   const label=(name+" "+String(a?.non_item_fee?.name??"")).toLowerCase();if(/балл|point|скидк/.test(label))points+=amount;
  }
