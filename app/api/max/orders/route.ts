...
import { sendMaxMessage, maxUserFromRequest } from '../../../../lib/max';
...
const orderNumber=()=>`TR-MAX-${new Date().toISOString().slice(2,10).replaceAll('-','')}-${crypto.randomUUID().slice(0,5).toUpperCase()}`;

export async function GET(request: Request) {
  const user=maxUserFromRequest(request); if(!user)return NextResponse.json({error:'Недействительная сессия MAX.'},{status:401});
  const limit=rateLimit(request,`max-orders-${user.id}`,60,60_000);if(!limit.ok)return NextResponse.json({error:'Слишком много запросов.'},{status:429});
  try { await ensureSchema(); const r=await getPool().query(`SELECT order_number,total_amount,status,delivery_method,payment_method,created_at FROM orders WHERE max_user_id=$1 ORDER BY created_at DESC LIMIT 30`,[user.id]); return NextResponse.json(r.rows.map(x=>({...x,total_amount:Number(x.total_amount)})),{headers:{'Cache-Control':'no-store'}}); }
  catch{return NextResponse.json({error:'Не удалось получить заказы.'},{status:503})}
}

export async function POST(request: Request) {
  const user=maxUserFromRequest(request); if(!user)return NextResponse.json({error:'Недействительная сессия Telegram.'},{status:401});
  const limit=rateLimit(request,`max-checkout-${user.id}`,8,60_000);if(!limit.ok)return NextResponse.json({error:'Слишком много попыток. Попробуйте позже.'},{status:429});
  try {
    ...
    const admin=process.env.MAX_ADMIN_USER_ID; if(admin)void sendMaxMessage(admin,`🛒 Новый заказ ${response.orderNumber}\nКлиент: ${customerName}\nТелефон: ${phone}\nСумма: ${response.totalAmount.toLocaleString('ru-RU')} ₽\nMAX: ${user.username?'@'+user.username:user.id}`).catch(()=>{});
